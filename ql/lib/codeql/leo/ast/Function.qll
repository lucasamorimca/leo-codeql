...
class Function extends AstNode, @leo_function {
   ...
   */
  predicate isConstructor() { this.getKind() = 4 }
...
/**
 * A constructor function (runs once at deploy time)
 */
class ConstructorFunction extends Function {
  ConstructorFunction() { this.isConstructor() }

  override string toString() { result = "constructor" }
}
...
class Parameter extends AstNode, @leo_parameter {
  ...
  /**
   * Gets the visibility of this parameter (0=private, 1=public, 2=constant)
   */
  ...
  /**
   * Checks if this parameter is constant
   */
  predicate isConstant() { this.getVisibility() = 2 }
